...
export class RobotCompanion {
  constructor(scene, x, y, savedRobot = {}) {
    ...
    this.memory = Math.max(2, Number(savedRobot.memory) || 2);
  ...
  update(player) {
    ...
    const distance = Phaser.Math.Distance.Between(this.sprite.x, this.sprite.y, desiredX, desiredY);
  ...
  gesture(type, duration = 1500) {
    ...
    const texture = { core: "holo-core", portal: "holo-portal", alert: "holo-alert" }[type] || "holo-alert";
    ...
    this.scene.tweens.add({
      ...
      yoyo: true,
      repeat: 2,
      onComplete: () => this.sprite?.active && this.sprite.setAngle(0),
    });
  }
