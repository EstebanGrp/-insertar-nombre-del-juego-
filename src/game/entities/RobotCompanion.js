...
export class GameScene extends Phaser.Scene {
  ...
  createRobotAndCampaignPart() {
    ...
    if (this.levelNumber === 1) {
      ...
      if (this.saveData.robot.discovered) this.robot.markDiscovered();
  ...
  createPortal() {
    ...
    this.portalMarker = this.add
      .text(this.layout.portal.x, this.layout.portal.y - 125, "▼", {
    ...
    if (this.performance.profile.backgroundTweens && this.settings.motion !== false) {
      this.tweens.add({
        targets: this.portalMarker,
        y: this.layout.portal.y - 139,
  ...
  updateInteractions() {
    ...
    const documentEntry = this.learningDocs.nearest(playerSprite, 150);
    if (documentEntry) {
  ...
  updatePortalState() {
    ...
    this.portalMarker?.setVisible(ready);
  }
